"""Pre-ship validation (spec §6).

1. Physics anchors: model carry vs hit_distance_sc on Mead's actual HRs
   (MAE must be < 15 ft, else refit KD/KL on his HR set).
2. Layer 2 calibration: reliability of KNN outcome probabilities on
   held-out RHB Fenway BBE.
...
Usage: uv run python model/validate.py [--plot-dir DIR]
"""
...
from fence import fence_at
from outcomes import OUTCOMES, FenwayOutcomeModel, layer1_fence_interaction, predict_fenway
from trajectory import trajectory

RAW = HERE.parent / "data" / "raw"


def check_physics_anchors(mead: pd.DataFrame) -> dict:
    hr = mead[(mead.outcome == "HR") & mead.hit_distance_sc.notna()]
    errs = []
    for _, r in hr.iterrows():
        carry, *_ = trajectory(r.launch_speed, r.launch_angle)
...
def check_calibration(fenway: pd.DataFrame, plot_dir: Path | None, seed: int = 7) -> dict:
    rhb = fenway[fenway["stand"] == "R"].reset_index(drop=True)
    rng = np.random.default_rng(seed)
    test_mask = rng.random(len(rhb)) < 0.2
    train, test = rhb[~test_mask], rhb[test_mask]

    model = FenwayOutcomeModel().fit(train)
    ...
    if plot_dir is not None:
        ...
        fig.suptitle("Layer 2 KNN reliability — held-out RHB Fenway BBE", fontsize=12)
        fig.tight_layout()
        fig.savefig(plot_dir / "calibration.png", dpi=150)
        stats["plot"] = str(plot_dir / "calibration.png")
    return stats


def check_hr_reconciliation(mead: pd.DataFrame, fenway: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "actual_hr": int((mead.outcome == "HR").sum()),
...
def main() -> None:
    ...
    mead = pd.read_parquet(RAW / "mead_2026_bbe.parquet")
    fenway = pd.read_parquet(RAW / "fenway_bbe.parquet")

    print("=== 1. Physics anchors (Mead HRs) ===")
    anchors = check_physics_anchors(mead)
    print(anchors)
    status = "PASS" if anchors["mae_ft"] < 15 else "FAIL — refit KD/KL"
    print(f"MAE {anchors['mae_ft']:.1f} ft -> {status}\n")

    print("=== 2. Layer 2 calibration (held-out Fenway RHB) ===")
    cal = check_calibration(fenway, args.plot_dir)
    print({k: v for k, v in cal.items() if not k.startswith("base_")}, "\n")

    print("=== 3+4. HR reconciliation & layer divergence ===")
    rec = check_hr_reconciliation(mead, fenway)
    print(rec)
    gap = abs(rec["expected_fenway_hr_blended"] - rec["physics_hand_count_tall_walls"])
    print(f"blended expected vs tall-wall hand count gap: {gap:.1f} -> {'PASS' if gap <= 3 else 'REVIEW'}")
